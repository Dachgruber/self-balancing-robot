...
void setup() {
  ...
  leftStep.setAcceleration(16000.0);
  rightStep.setAcceleration(16000.0);
...
ISR(TIMER1_COMPA_vect) {
  ...
  //Serial.print("ISR ");
  leftStep.run(); //calling run instead of runSpeed tells the steppers to acc to maxSpeed!
  rightStep.run();
...
void computePID() {  
  ...
  motorPower = constrain(output, -360, 360); //again, damit uns die Geschichte nicht um die Ohren fliegt. 360 für 360°
