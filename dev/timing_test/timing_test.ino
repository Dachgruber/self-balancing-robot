...
const int maxSpeedLimit = 8000.00;
const int speedRange = 4000;
const int toleranceRange = 600;
...
void computePID() {  
  ...
  motorPower = constrain(output, -toleranceRange, toleranceRange); //again, damit uns die Geschichte nicht um die Ohren fliegt
...
void setMotors(float leftSpeed, float rightSpeed){
  ...
  float leftmSpeed = map(leftSpeed, -toleranceRange, toleranceRange, -speedRange, speedRange);
  float rightmSpeed = map(rightSpeed, -toleranceRange, toleranceRange, -speedRange, speedRange);
  
  //constrain damit die Stepper nicht platzen
  //technically not needed, but the AccelStepper Lib doesnt like huge numbers
  leftmSpeed = constrain(leftmSpeed, -maxSpeedLimit, maxSpeedLimit);
  rightmSpeed = constrain(rightmSpeed,-maxSpeedLimit, maxSpeedLimit);
